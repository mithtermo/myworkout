...
const WORKOUT_TYPES = [
  { value:'gym_push',       label:'🏋️ Gym — Push',       desc:'Chest, shoulders (Sun/Sat)' },
  { value:'gym_pull',       label:'🏋️ Gym — Pull',       desc:'Back, biceps (Mon)' },
  { value:'gym_legs',       label:'🦵 Gym — Legs+Core',  desc:'Legs, core (Tue)' },
  { value:'gym_metabolic',  label:'🔥 Gym — Metabolic',  desc:'Full body circuit (Wed)' },
  { value:'gym_upper',      label:'💪 Gym — Upper',      desc:'Upper body (Thu if no footy)' },
  { value:'home_resistance',label:'🏠 Home — Resistance',desc:'Barbell/dumbbell at home' },
  { value:'home_cardio',    label:'🚲 Home — Cardio',    desc:'Elliptical or chest expander' },
  { value:'football',       label:'⚽ Football',          desc:'Thursday football (90 min)' },
  { value:'badminton',      label:'🏸 Badminton',         desc:'Badminton session' },
];
...
const EXERCISE_SUGGESTIONS = {
  ...
  home_cardio:    ['Elliptical','Chest expander','Jumping jacks'],
};
...
export default function LogWorkout() {
  const [form, setForm]     = useState({ date:today, time:now, type:'', duration_min:'', pre_bg:'', post_bg:'', notes:'' });
  ...
  const handleSubmit = async e => {
    ...
    if (!error) { setEx([]); setForm(f=>({...f, duration_min:'', pre_bg:'', post_bg:'', notes:''})); }
    setSaving(false);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">Record your session with exercises and blood glucose data.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="card">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><label className="label">Start Time</label><input type="time" className="input" value={form.time} onChange={e=>set('time',e.target.value)} /></div>
            <div><label className="label">Duration (min)</label><input type="number" min="5" max="240" className="input" placeholder="e.g. 60" value={form.duration_min} onChange={e=>set('duration_min',e.target.value)} /></div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {WORKOUT_TYPES.map(t=>(
              <button key={t.value} type="button" onClick={()=>set('type',t.value)}
                className={`p-3 rounded-xl border text-left text-sm transition-colors ${form.type===t.value?'border-brand-400 bg-brand-50 text-brand-700':'border-gray-200 bg-white text-gray-600 hover:bg-gray-50'}`}>
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.1" min="1" max="30" className="input" placeholder="e.g. 6.5" value={form.pre_bg} onChange={e=>set('pre_bg',e.target.value)} />
            ...
            <div>
              ...
              <input type="number" step="0.1" min="1" max="30" className="input" placeholder="e.g. 5.2" value={form.post_bg} onChange={e=>set('post_bg',e.target.value)} />
        ...
        {(form.type.startsWith('gym_')||form.type.startsWith('home_')) && (
          <div className="card">
            ...
            {exercises.length > 0
              ? <div className="space-y-2">
                  <div className="grid grid-cols-12 gap-2 text-xs text-gray-400 px-0.5">
                    <span className="col-span-4">Exercise</span><span className="col-span-2">Sets</span><span className="col-span-2">Reps</span><span className="col-span-2">Load(kg)</span>
                  </div>
                  {exercises.map((ex,i)=><ExerciseRow key={i} ex={ex} onChange={v=>updateEx(i,v)} onRemove={()=>removeEx(i)} />)}
                </div>
              : <p className="text-xs text-gray-400 text-center py-4">Use quick-add or click "+ Add Exercise".</p>
            }
          </div>
        )}
        ...
        <div className="card">
          ...
          <textarea className="input min-h-[70px] resize-none" placeholder="e.g. Felt strong today. Increased bench to 60kg…"
            value={form.notes} onChange={e=>set('notes',e.target.value)} />
        </div>

        <button type="submit" className="btn-primary w-full text-base py-3" disabled={saving||!form.date||!form.type}>
